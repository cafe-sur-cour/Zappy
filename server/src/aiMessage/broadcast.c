...
static void print_broadcast_server(player_t *source, player_t *dest, int orientation)
{
    int len = int_str_len(source->id) + int_str_len(dest->id) +
        int_str_len(dest->posX) + int_str_len(dest->posY) + 1 + 57;
    char *debug = calloc(len + 1, sizeof(char));

    if (!debug) {
        error_message("Memory allocation failed for broadcast debug print.");
        return;
    }
    snprintf(debug, len + 1, "Player (%d) broadcasted to player (%d) at (%d, %d). Orientation: %d",
