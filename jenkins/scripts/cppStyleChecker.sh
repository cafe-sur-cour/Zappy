...
find . -type f \( -name "*.cpp" -o -name "*.hpp" \) ! -path "*/tests/*" ! -path "*/bonus/*" ! -path "*/documentation/*" ! -path "*/temp/*" | while read -r file; do
    cpplint --filter="$FILTERS,$NOFILTERS" --linelength=$LINELENGTH "$file" 2>> "$LOG_FILE"
done

echo "C++ Coding style check completed. See $LOG_FILE for details."
