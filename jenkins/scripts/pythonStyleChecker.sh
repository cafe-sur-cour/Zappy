...
find ./ai -type f -name "*.py" ! -path "*/tests/*" ! -path "*/bonus/*" ! -path "*/__pycache__/*" ! -path "*venv*" ! -path "*/documentation/*" | while read -r file; do
    echo "Checking $file..."
...
if [ "$ERROR_COUNT" -eq 0 ]; then
    echo "Python coding style check completed. No errors found!"
    rm "$LOG_FILE"  # Remove the log file if empty
else
    echo "Python coding style check completed. Found $ERROR_COUNT error(s). See $LOG_FILE for details."
fi
