...
// Test for sgt command

Test(sgt, valid_command, .init = redirect_all_std)
{
    ...
    result = sgt(zappy, zappy->graph, message);
    cr_assert_eq(result, 0);
    fgets(buffer, sizeof(buffer), fp);
    fclose(fp);
    remove("gui_socket");
    cr_assert_str_eq(buffer, "sgt 100\n");
}

Test(sgt, invalid_command, .init = redirect_all_std)
{
    zappy_t *zappy = default_zappy();
    char message[] = "sgt rgefhjd\n";
    int result;
...
Test(sgt, invalid_file_descriptor, .init = redirect_all_std)
{
    zappy_t *zappy = default_zappy();
    char message[] = "sgt\n";
    int result;
